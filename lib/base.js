(function (window) {
    ...
    class Preloader {
        ...
        hideSpinner() {
            let spinner = this.getSpinner();
            Modal.removeElementDOM(spinner);
            return this;
        }
        show() {
        ...
        hide() {
            let preloader = this.getPreloader();
            Modal.removeElementDOM(preloader);
            return this;
        }
        status() {
    ...
    class Modal {
        ...
        setTitle(text) {
            let title = this.getTitle(), node = typeof text !== 'object' || text === null ? document.createTextNode(text) : text;
            title.innerHTML = '';
        ...
        getDialog() {
            let have = this.haveDialog();
            if (have) return this.elements.dialog;
            let size = this.getSize();
            this.elements.dialog = document.createElement('div');
            this.elements.dialog.className = 'trinity pure-u-' + size + '-24';
